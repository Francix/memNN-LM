...
class MemN2N(object):
    def __init__(self, config, sess):
        ...
        # Question: what is mem_size? 
        # -- use word[i - mem_size: i] to predict word[i]
        self.mem_size = config.mem_size
        # Question: what is lindim? 
        ...
        # Note: here the input is fixed to a constant vector 
    ...
    def build_memory(self):
        ...
        Ain_c = tf.nn.embedding_lookup(self.A, self.context) # shape = [batch_size, mem_size, edim]
        Ain_t = tf.nn.embedding_lookup(self.T_A, self.time)  # shape = [batch_size, mem_size, edim]
        Ain = tf.add(Ain_c, Ain_t)                           # tf.add(a, b) = a + b, shape as the same 

        # c_i = sum B_ij * u + T_B_i
        Bin_c = tf.nn.embedding_lookup(self.B, self.context) # shape = [batch_size, mem_size, edim]
        Bin_t = tf.nn.embedding_lookup(self.T_B, self.time)  # shape = [batch_size, mem_size, edim]
        Bin = tf.add(Bin_c, Bin_t)                           # shape = [batch_size, mem_size, edim]

        for h in xrange(self.nhop):
            self.hid3dim = tf.reshape(self.hid[-1], [-1, 1, self.edim])   # [batch_zize, edim] -> [batch_size, 1, edim]
            Aout = tf.matmul(self.hid3dim, Ain, adjoint_b = True)         # [batch_size, 1, edim] * [batch_size, edim, mem_size] -> [batch_size, 1, mem_size]
            Aout2dim = tf.reshape(Aout, [-1, self.mem_size])              # shape = [batch_size, mem_size]
            P = tf.nn.softmax(Aout2dim)                                   # probability distribution over memory, i.e. attention 

            probs3dim = tf.reshape(P, [-1, 1, self.mem_size])             # shape = [batch_size, 1, mem_size]
            # perform attention on Bin i.e. context 
            Bout = tf.matmul(probs3dim, Bin)                              # [batch_size, 1, mem_size] * [batch_size, mem_size, edim] -> [batch_size, 1, mem_size]
            Bout2dim = tf.reshape(Bout, [-1, self.edim])                  # shape = [batch_size, edim]

            Cout = tf.matmul(self.hid[-1], self.C)                        # [batch_zize, edim] * [edim, edim] -> [batch_size, edim] 
    ...
    def build_model(self):
        ...
        # W from hidden dimension to the entire word vocabulary
    ...
    def train(self, data):
        ...
        for idx in xrange(N):
            ...
            target.fill(0)
            # for every training case in the batch
            for b in xrange(self.batch_size):
                # randomly chose a target word 
                m = random.randrange(self.mem_size, len(data))
                target[b][data[m]] = 1
                # previous self.mem_size words 
            ...
            cost += np.sum(loss)
            if(idx % 20 == 0): print("step loss: %.4f" % np.mean(loss))
